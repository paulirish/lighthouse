...
app.get('/logs', (req, res) => {
  if (!proc) {
    res.write('no process yet. reload later');
    return res.end();
  }
  res.writeHead(200);
  res.write('streaming stderr and stdout..\n');
...
app.get('/', (req, res) => {
  if (isBusy) {
    ...
    isBusy = true;
    try {
      const cmd = `lighthouse --port 9222 --output-path=../report.${req.query.format} --output=${req.query.format} ${req.query.url}`;
      ...
      proc = exec(cmd, {
        ...
        error => {
          if (error !== null) {
            ...
            res.write(new Buffer(JSON.stringify(error), 'utf8'));
            res.end();
            return;
          }
...
console.log(`Running on https://localhost:${HTTPS_PORT} and http://localhost:${HTTP_PORT}`);
